...
export const RigthMenu = () => {
  ...
  const opponentOnPage = useSelector((state: any) => state.game.opponentOnPage) || true;
  ...
  const strikedBlack = strikedFigures.black.map((figure: string) =>
    <div className={'game__right-menu__striked__figure' + ` ${figure.replace(/\d/, '')}` + ' b'} key={`${figure}-b`}></div>
  );
  const strikedWhite = strikedFigures.white.map((figure: string) =>
    <div className={'game__right-menu__striked__figure' + ` ${figure.replace(/\d/, '')}` + ' w'} key={`${figure}-w`}></div>
  );
